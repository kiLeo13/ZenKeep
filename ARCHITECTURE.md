...
- The websocket manager imports `useWebSocket` from `react-use-websocket/dist/lib/use-websocket` and `ReadyState` from `react-use-websocket/dist/lib/constants`. Keep those implementation imports unless the package ships a production-safe ESM entrypoint; Vite 8/Rolldown can mis-bundle the package index default export and produce a production-only `default is not a function` crash on the authenticated home route.
