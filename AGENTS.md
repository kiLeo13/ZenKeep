...
- Successful company lookup responses expose `cached=false` when fulfilled by the external lookup provider and `cached=true` when fulfilled by the local `companies` cache. The frontend company lookup modal allows repeating the currently displayed CNPJ while that displayed result is a cache miss, so the header cache icon can update after the backend returns a cache hit.
